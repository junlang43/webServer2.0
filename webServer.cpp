...
int WebServer::run() {
	...
	//port reuse
	int flag = 1;
	setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR, (const void *)&flag, sizeof(flag));
    ...
    epoll_ctl( epoll_fd, EPOLL_CTL_ADD, sock_fd, &event );//add listen socket to epoll tree
    ...
    while( true ) {
        ...
        for( int i = 0; i < ret; i++ ) {
            ...
            if( fd == sock_fd ) {  //新连接
				//fd=listen sicket, add connect socket to epoll tree
